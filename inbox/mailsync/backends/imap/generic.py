...
GenericUIDMetadata = namedtuple('GenericUIDMetadata', 'size')
...
class FolderSyncEngine(Greenlet):
    ...
    def initial_sync_impl(self, crispin_client):
        ...
        try:
            ...
            self.batch_download_uids(crispin_client, uids)
    ...
    def should_idle(self, crispin_client):
        return ('IDLE' in crispin_client.conn.capabilities() and
    ...
    def batch_download_uids(self, crispin_client, uids, sizes=None):
        ...
        while uids():
            uid = uids.popleft()
            # The speedup from batching here is much less clear for various
            # generic IMAP backends than it is for Gmail, so just download
            # one-at-a-time for now.
            self.download_and_commit_uids(crispin_client, [uid])
            report_progress(self.account_id, self.folder_name, 1, len(uids))
            self.heartbeat_status.publish()
    ...
    def get_new_uids(self, crispin_client):
        ...
        if remote_uidnext != self.uidnext:
            ...
            new_uid_data = crispin_client.conn.fetch(
                '{}:*'.format(self.uidnext), ['RFC822.SIZE'])
            new_uid_sizes = {k: v.get('RFC822.SIZE') for k, v in
                             new_uid_data.items()}
            new_uids = sorted(new_uid_data.keys())
            self.batch_download_uids(crispin_client, new_uids, new_uid_sizes)
    ...
    def condstore_refresh_flags(self, crispin_client):
        ...
        crispin_client.select_folder(self.folder_name, self.uidvalidity_cb)
        changed_flags = crispin_client.changed_flags(self.highestmodseq)
        remote_uids = crispin_client.all_uids()
        with session_scope() as db_session:
