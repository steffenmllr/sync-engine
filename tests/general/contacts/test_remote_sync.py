...
def test_add_contacts(contacts_provider, contact_sync, db):
    ...
    num_original_local_contacts = db.session.query(Contact).filter_by(
        namespace_id=NAMESPACE_ID).count()
    ...
    contact_sync.poll()
    num_current_local_contacts = db.session.query(Contact).filter_by(
        namespace_id=NAMESPACE_ID).count()
...
def test_update_contact(contacts_provider, contact_sync, db):
    ...
    contact_sync.provider_instance = contacts_provider
    contact_sync.poll()
    results = db.session.query(Contact).filter_by(
